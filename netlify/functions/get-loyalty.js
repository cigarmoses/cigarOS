...
function parseCSV(text){
  ...
  return rows.slice(1).map(row => {
    const o={}; head.forEach((k,idx)=>o[k]=row[idx]??''); return o;
  });
}
...
function findHeader(o, token){
  ...
  const exact = ks.find(k => k.toLowerCase()===t || k.toLowerCase().includes(`${t} name`));
...
function splitName(s){
  ...
  const p=s.split(/\s+/); if(p.length>=2) return {first:p.slice(0,-1).join(' '), last:p.slice(-1)[0]};
  return {first:'', last:s};
}
...
/* ---------- loaders (blobs → csv/json, then repo → csv/json) ---------- */
...
async function loadFromRepo(){
  // try repo CSV first (since that’s what you committed)
  try{
    const urlCSV = new URL('../../img/contacts.csv', import.meta.url);
    const resCSV = await fetch(urlCSV);
    if (resCSV.ok){
      const text = await resCSV.text();
      const arr = parseCSV(text);
      if (arr.length) return {source:'repo-csv', data: arr.map(normalizeRecord)};
    }
  }catch{}

  // then repo JSON
  try{
    const urlJSON = new URL('../../img/contacts.json', import.meta.url);
    const resJSON = await fetch(urlJSON);
    if (resJSON.ok){
      const arr = await resJSON.json();
      if (Array.isArray(arr)) return {source:'repo-json', data: arr.map(normalizeRecord)};
    }
  }catch{}

  return {source:'repo-none', data:[]};
}
...
export default async () => {
  try{
    ...
    if (!data.length){
      const fallback = await loadFromRepo();
      source = fallback.source;
      data = fallback.data;
    }
    if (!data.length) throw new Error('No contacts found in blobs or /img');
